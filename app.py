...
def animate_plot(
    ...
    total_points = len(result.x)
    frame_step = max(1, total_points // 200)
    frame_ends = list(range(2, total_points + 1, frame_step))
    if frame_ends[-1] != total_points:
        frame_ends.append(total_points)

    def render(end_idx: int) -> None:
        fig, ax = plt.subplots(figsize=(7, 4.5))
        ax.plot(result.x[:end_idx], result.y[:end_idx], label=title, linewidth=2)
        ax.scatter(result.x[end_idx - 1], result.y[end_idx - 1], s=30)
        if extra_line is not None:
            extra_x, extra_y, lbl = extra_line
            ax.plot(extra_x, extra_y, "--", label=lbl, alpha=0.8)
        ax.set_xlabel("x (m)")
...
def main() -> None:
    ...
    for tab, (name, result) in zip(tabs, metrics.items()):
        with tab:
            ...
            animate_now = st.button("Play Animation", key=f"animate_{name}")
            if name == "Inclined Plane":
                ...
                animate_plot(
                    name,
                    result,
                    extra_line=(local_x, local_y, "Inclined Surface"),
                    animate=animate_now,
                )
            else:
                animate_plot(name, result, animate=animate_now)
